...
//     let total_month_width = 50; // Total width for all months
...
//                 let r_match = data_str.match(/R:\s*₹?\s*R\.?\s*([\d,.]+)/);  // Handle "R: ₹ R. 0.00"
//                 let a_match = data_str.match(/A:\s*₹?\s*A\.?\s*([\d,.]+)/);  // Handle "A: ₹ A. 0.00"

//                 if (q_match) qty = q_match[1];
//                 if (r_match) {
//                     rate = r_match[1];
//                 } else {
//                     // Try simpler R: pattern
...
<body>
	...
	let month_width =
		month_columns.length > 0 ? Math.floor(total_month_width / month_columns.length) : 20;
	...
	data.forEach((row, index) => {
		...
		// Check if this is a HEADER row (Summary/Total with month names in columns)
		let is_header =
			clean_first_col.toLowerCase().includes("summary") ||
			clean_first_col.toLowerCase().includes("total");

		// Check if this row has month column headers instead of data
		let has_month_headers = false;
		if (is_header && month_columns.length > 0) {
			let first_month_val = String(row[month_columns[0].fieldname] || "")
				.replace(/<[^>]*>/g, "")
				.trim();
			if (
				first_month_val &&
				!first_month_val.includes("Q:") &&
				!first_month_val.includes("|")
			) {
				has_month_headers = true;
			}
		}
		...
		if (has_month_headers) {
			...
			month_columns.forEach((col) => {
				let header_val = String(row[col.fieldname] || "")
					.replace(/<[^>]*>/g, "")
					.trim();
				html += `
                    <td style="font-weight: bold; text-align: center; border: 1px solid #333;">${
						header_val || col.label
					}</td>`;
			});

			// Add Cost header
			let cost_field = columns[columns.length - 2].fieldname;
			let cost_header = String(row[cost_field] || "")
				.replace(/<[^>]*>/g, "")
				.trim();
			html += `
                    <td style="font-weight: bold; text-align: center; border: 1px solid #333;">${
			...
			// Add Total header
			let total_field = columns[columns.length - 1].fieldname;
			let total_header = String(row[total_field] || "")
				.replace(/<[^>]*>/g, "")
				.trim();
			html += `
                    <td style="font-weight: bold; text-align: center; border: 1px solid #333;">${
						total_header || "Total Amount"
					}</td>
                </tr>`;
			return;
		}
		...
		// Cost column
		let cost_field = columns[columns.length - 2].fieldname;
		let cost_value = String(row[cost_field] || "0.00")
			.replace(/<[^>]*>/g, "")
			.replace(/₹/g, "")
			.trim();
		html += `
		...
		// Total
		let total_field = columns[columns.length - 1].fieldname;
		let total_value = String(row[total_field] || "0.00")
			.replace(/<[^>]*>/g, "")
			.replace(/₹/g, "")
			.trim();
